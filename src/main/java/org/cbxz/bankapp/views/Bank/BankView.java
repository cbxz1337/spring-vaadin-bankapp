...
public class BankView extends VerticalLayout {
  ...
  private final HorizontalLayout tools = new HorizontalLayout(filter, deleteButton,
      showCreditDetails, showScheduleDetails);
  ...
  private Dialog scheduleDetails() {
    ...
    try {
      Iterable<Schedule> schedulesIterable = scheduleRepo
          .findByClientIdAndCreditId(bank.getClient().getId(), bank.getCredit().getId());
  ...
  private Dialog showCreditDetails() {
    ...
    try {
      ...
      CreditOffer creditOffer = creditOfferRepo
          .findByCreditAndClient(bank.getCredit(), bank.getClient());
      double sumWithPercent =
          creditOffer.getSum() * bank.getCredit().getPercent() / 100 + creditOffer.getSum();
      ...
      creditDetailsLabel.getStyle()
          .set("border", "none")
          .set("border-bottom", "1px white solid");
      verticalLayout.add(label, creditDetailsLabel, clientsLabel, clientDetailsLabel);
      dialog.add(verticalLayout);
    } catch (Exception e) {
      e.printStackTrace();
  ...
  private void deleteBank() {
    ...
    try {
      creditOfferRepo
          .delete(creditOfferRepo.findByCreditAndClient(bank.getCredit(), bank.getClient()));
      scheduleRepo.deleteAll(scheduleRepo
          .findByClientIdAndCreditId(bank.getClient().getId(), bank.getCredit().getId()));
