...
public class ClientEditor extends Dialog implements KeyNotifier {
  ...
  public ClientEditor(ClientsRepository clientsRepository) {
    ...
    save.addClickListener(e -> {
      if (fieldValidator()) {
        ...
        Notification.show("Вы не заполнили все необходимые поля")
  ...
  public void save() {
    try {
      Optional<Client> clientOptional = clientsRepository
          .findByPassportNumber(passportNumber.getValue());
      if (clientOptional.isEmpty()) {
        clientsRepository.save(client);
        changeHandler.onChange();
      } else {
        Notification.show("Клиент с таким паспортом уже сущестует.")
            .setPosition(Notification.Position.MIDDLE);
      }
      changeHandler.onChange();
    } catch (Exception e) {
